...
plot_sev = predicted %>% 
  ...
  geom_line(data = predicted, aes(year, mean_efficacy), size = 1.7, color = "#E05313")+
...
plot_yld = predicted_yld %>% 
 # mutate(brand_name = factor(ai, levels = c("BIX + PROT + TRIFL","FLUX + PYRA"))) %>%
  ...
  geom_line(data = predicted_yld, aes(year, mean_gain), size = 1.7, color = "#4D1C06")+
...
ggsave("fig/decline_efficacy_sev_yld.png", width = 8, height = 6, dpi = 600, bg = "white")
